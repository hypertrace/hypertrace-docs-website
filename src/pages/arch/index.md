...
This [config service](https://github.com/hypertrace/config-service) is designed to be a single macroservice, contained in one repo with separate modules defining different GRPC services that expose feature-specific APIs. The query and config logical group separation you see in the architecture is a step towards the next iteration of architecture where you will see 3 logical groups namely, ingestion, config and Query layers. 
